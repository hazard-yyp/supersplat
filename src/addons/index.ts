...
/**
 * 你需要传入的 app 接口来自 supersplat-adapter：
...
export function setup(app: any) {
  ...
  const tick = () => {
    try {
      ...
      // 7.1 轻量粗过滤：仅处理半径 R 内的格子（后续可换成视锥裁剪）
      const R = params.midDist * 1.5;
      const cx = cam.position[0], cy = cam.position[1], cz = cam.position[2];
      ...
      const draw = lodEnabled
        ? lod.filter(cellsNearby, cam, estimatePx)
        : allIndices;

      // 7.3 写回渲染端（适配器内部有节流 + 增量）
